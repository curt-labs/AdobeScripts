/**************************************
    ...
    This script will export all the artboards in an AI file to individual PNG files.
    
    It also scales the images to desired percentage, allows you to prefix the file names and specify output directory.
...
if ( nyt_png_info.textFrames.length != 1 ) {
    ...
    try {
        ...
        nyt_png_scaling   = nyt_png_xml.nyt_scaling;
...
function nyt_show_png_dialog() {
    ...
    // SCALING GRP
    var scalingGrp = nyt_png_dlg.msgPnl.add('group',undefined,'')
    scalingGrp.orientation = 'row';
    scalingGrp.alignment = [ScriptUI.Alignment.LEFT, ScriptUI.Alignment.TOP]
    
    var scalingSt = scalingGrp.add('statictext', undefined, 'Scaling Percentage:');
    scalingSt.size = [100,20];
    
    var scalingEt = scalingGrp.add('edittext', undefined, nyt_png_scaling);
    scalingEt.size = [300,20];
    ...
    nyt_png_dlg.btnPnl.okBtn.onClick = function() { 
        ...
        nyt_png_scaling = scalingEt.text;
...
function nyt_run_export( num_to_export ) {
    ...
    if(nyt_png_scaling == undefined || nyt_png_scaling == 0){
        nyt_png_scaling = 100.0;
    }
    ...
    for (var i = 0; i < num_artboards; i++ ) {
        ...
        // if ( ! ( artboardName.match(  /^artboard/ ) || artboardName.match( /^\-/ ) )){
            ...
            options.verticalScale = nyt_png_scaling;
            options.horizontalScale = nyt_png_scaling;
            options.matte = true;
    ...
    nyt_png_xml.nyt_scaling = nyt_png_scaling;
